...
var DbSet = {
    ...
    delByid:function(obj,req,res,logMsg){
        var targetId = req.body.id;
        if(shortid.isValid(targetId)){
            obj.remove({_id:targetId})
                .then(function(){
                    console.log(logMsg + 'success');
                    res.end('success');
                }).catch(function(err){
    ...
    //通用单条件查询的操作
    findByCondition:function(obj,req,res,condition,logMsg){
        obj.find(condition)
            .then(function(data){
                if(data.length < 1){
                    res.json({result:-1})
    ...
    //通用的多条件查询的操作
    findByConditions:function(obj,req,res,conditions,logMsg){
        obj.find(conditions)
            .then(function(data){
                if(data.length < 1){
                    res.json({result:-1})
    ...
    //通用的单条件只查询结果不返回数据
    findExists:function(obj,req,res,conditions,logMsg){
    	if(conditions){
    		obj.find(conditions).count()
    		.then(function(count){
    			if(count > 0){
    				res.json({result:1})
    			}else{
    ...
    // 带条件更新一条
    updateOneByCondition:function(obj,req,res,conditions,logMsg){
